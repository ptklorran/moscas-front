<template>
  <layout :style="`background: ${$theme.background}`" :title="$route.name" :options="options">
    ...
    >
      <VisualizacaoEmCard
        v-show="get_funcionarios.docs.length > 0"
        v-for="payload in get_funcionarios.docs"
        :key="payload.id"
        :payload="payload"
      />
    </div>
    ...
    <div v-else class="expande-horizontal wrap">
      <VisualizacaoEmLista v-show="get_funcionarios.docs.length > 0" />
    </div>

    <ListagemVazia v-show="get_funcionarios.docs.length === 0" />
    <Paginacao />
    <ModalView />
      ...
      <div slot="pdf-content">
        <section class="pdf-item">
          ...
          <div v-if="card" class="expande-horizontal centraliza pa-6 wrap">
            <VisualizacaoEmCard
              v-show="get_funcionarios.docs.length > 0"
              v-for="payload in get_funcionarios.data"
              :key="payload.id"
              :payload="payload"
            />
          </div>
...
export default {
      ...
      options: [
        {
          ...
          action: () => this.abre_modal_view_funcionario()
        },
        {
          icon: "mdi-update",
          nome: "Atualizar",
          action: () => this.listar_funcionarios()
        },
        {
          icon: "mdi-printer",
          nome: "Imprimir",
  ...
  methods: {
    ...mapActions(["abre_modal_view_funcionario", "listar_funcionarios"]),
    imprimir() {
      this.card = false;
