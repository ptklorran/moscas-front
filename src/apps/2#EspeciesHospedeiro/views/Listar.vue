<template>
  ...
  >
    ...
    >
      <VisualizacaoEmCard
        v-show="get_especies_hospedeiros.docs.length > 0"
        v-for="payload in get_especies_hospedeiros.docs"
        :key="payload.id"
        :payload="payload"
      />
    </div>
    ...
    <div v-else class="expande-horizontal wrap">
      <VisualizacaoEmLista v-show="get_especies_hospedeiros.docs.length > 0" />
    </div>

    <ListagemVazia v-show="get_especies_hospedeiros.docs.length === 0" />
    <Paginacao />
    <ModalView />
      ...
      <div slot="pdf-content">
        <section class="pdf-item">
          ...
          <div v-if="card" class="expande-horizontal centraliza pa-6 wrap">
            <VisualizacaoEmCard
              v-show="get_especies_hospedeiros.docs.length > 0"
              v-for="payload in get_especies_hospedeiros.data"
              :key="payload.id"
              :payload="payload"
            />
          </div>
      ...
      options: [
        {
          icon: "mdi-plus",
          nome: "Nova espécie",
          action: () => this.abre_modal_view_especie_hospedeiro()
        },
        {
          icon: "mdi-update",
          nome: "Atualizar",
          action: () => this.listar_especies_hospedeiros()
        },
        {
          icon: "mdi-printer",
          nome: "Imprimir",
