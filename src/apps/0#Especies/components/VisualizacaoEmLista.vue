      ...
      <template v-slot:default>
        ...
        <tbody>
          <tr
            ...
            "
            v-for="(item, i) in get_especies.docs"
            :key="item.id"
          >
            ...
            <td @click="abre_modal_view_especie(item)">{{ item.nome }}</td>
            <td @click="abre_modal_view_especie(item)">{{ item.genero }}</td>
            <td style="width: 10px">
              <v-btn
                @click="
                  createConfirmAction({
...
export default {
  computed: {
    ...mapGetters(["get_especies"])
  },
  methods: {
    ...mapActions(["abre_modal_view_especie", "createConfirmAction"]),
    setMarcado(event, item) {
      console.log("event", event);
      // this.add_especie_marcado(item)
    }
  }
};
</script>
