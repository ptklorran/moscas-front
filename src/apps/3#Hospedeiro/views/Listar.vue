<template>
  <layout :style="`background: ${$theme.background}`" :title="$route.name" :options="options">
    ...
    >
      <VisualizacaoEmCard
        v-show="get_hospedeiros.docs.length > 0"
        v-for="payload in get_hospedeiros.docs"
        :key="payload.id"
        :payload="payload"
      />
    </div>
    ...
    <div v-else class="expande-horizontal wrap">
      <VisualizacaoEmLista v-show="get_hospedeiros.docs.length > 0" />
    </div>

    <ListagemVazia v-show="get_hospedeiros.docs.length === 0" />
    <Paginacao />
    <ModalView />
      ...
      <div slot="pdf-content">
        <section class="pdf-item">
          ...
          <div v-if="card" class="expande-horizontal centraliza pa-6 wrap">
            <VisualizacaoEmCard
              v-show="get_hospedeiros.docs.length > 0"
              v-for="payload in get_hospedeiros.data"
              :key="payload.id"
              :payload="payload"
            />
          </div>
...
export default {
  data() {
    return {
      ...
      options: [
        {
          icon: "mdi-plus",
          nome: "Cadastrar Hospedeiro",
          action: () => this.abre_modal_view_hospedeiro()
        },
        {
          icon: "mdi-update",
          nome: "Atualizar",
          action: () => this.listar_hospedeiros()
        },
        {
          icon: "mdi-printer",
          nome: "Imprimir",
  ...
  computed: {
    ...mapGetters(["get_hospedeiros", "get_hospedeiros_filtros", "getLoggedUser"])
  },
  methods: {
    ...mapActions(["abre_modal_view_hospedeiro", "listar_hospedeiros"]),
    imprimir() {
      this.card = false;
