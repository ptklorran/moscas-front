<template>
  <layout :style="`background: ${$theme.background}`" :title="$route.name" :options="options">
    ...
    >
      <VisualizacaoEmCard
        v-show="get_ocorrencias.docs.length > 0"
        v-for="payload in get_ocorrencias.docs"
        :key="payload.id"
        :payload="payload"
      />
    </div>
    ...
    <div v-else class="expande-horizontal wrap">
      <VisualizacaoEmLista v-show="get_ocorrencias.docs.length > 0" />
    </div>
      ...
      <div slot="pdf-content">
        <section class="pdf-item">
          ...
          <div v-if="card" class="expande-horizontal centraliza pa-6 wrap">
            <VisualizacaoEmCard
              v-show="get_ocorrencias.docs.length > 0"
              v-for="payload in get_ocorrencias.data"
              :key="payload.id"
              :payload="payload"
            />
          </div>
...
export default {
  data() {
    return {
      ...
      options: [
        {
          icon: "mdi-plus",
          nome: "Cadastrar Ocorrência",
          action: () => this.abre_modal_view_ocorrencia()
        },
        {
          icon: "mdi-update",
          nome: "Atualizar",
          action: () => this.listar_ocorrencias()
        },
        {
          icon: "mdi-printer",
          nome: "Imprimir",
  ...
  computed: {
    ...mapGetters([
      "get_ocorrencias",
      "get_ocorrencias_filtros",
      "getLoggedUser"
    ])
  },
  methods: {
    ...mapActions(["abre_modal_view_ocorrencia", "listar_ocorrencias"]),
    imprimir() {
      this.card = false;
